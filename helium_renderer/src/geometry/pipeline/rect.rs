...
use crate::geometry::{buffer::{BufferGroup, BufferGroupBuilder}, uniform::UniformBuilder, vertex::Vertex};
...
impl RectPipeline {
	pub fn new(
		...
		let bounds_group = 
			BufferGroupBuilder::new("Rect")
			.add_buffer("Corner radius", size_of::<f32>(), wgpu::BufferUsages::UNIFORM | wgpu::BufferUsages::COPY_DST)
			.add_buffer("Size", size_of::<[f32;2]>(), wgpu::BufferUsages::UNIFORM | wgpu::BufferUsages::COPY_DST)
			.add_buffer("Position", size_of::<[f32;2]>(), wgpu::BufferUsages::UNIFORM | wgpu::BufferUsages::COPY_DST)
			.build(device);
		...
		let pipeline_layout = 
			device.create_pipeline_layout(
				&wgpu::PipelineLayoutDescriptor{
					...
					bind_group_layouts: &[window_uniform.layout(),bounds_group.bind_group_layout()],
